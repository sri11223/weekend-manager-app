...
import { EnhancedActivity } from '../../types/enhanced'
import { useTheme } from '../../hooks/useTheme'
import { apiIntegrationService } from '../../services/apiIntegrationService'
import { enhancedMockService } from '../../services/enhancedMockService'

interface EnhancedFloatingActivityBrowserProps {
  category: string
  onClose: () => void
  searchQuery?: string
  onAddActivity: (activity: EnhancedActivity, timeSlot?: string, day?: 'saturday' | 'sunday') => void
}

interface DraggableActivityCardProps {
  activity: EnhancedActivity
  onAddActivity: (activity: EnhancedActivity) => void
  theme: any
}
      ...
      <div className="space-y-2">
        ...
        {activity.moodTags && activity.moodTags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {activity.moodTags.slice(0, 2).map((tag: string, index: number) => (
              <span
                key={index}
...
const EnhancedFloatingActivityBrowser: React.FC<EnhancedFloatingActivityBrowserProps> = ({
  category,
  onClose,
  ...
  const loadActivities = async () => {
    try {
      ...
      if (searchQuery.trim()) {
        ...
        ).map(activity => ({ ...activity, title: activity.name, moodTags: activity.tags }))
        ...
        const enhancedMockActivities = mockActivities.map(activity => ({ ...activity, title: activity.name, moodTags: activity.tags }))
        result = [...enhancedMockActivities, ...apiActivities]
      } else {
        // Get all activities
        const allActivities = await enhancedMockService.getAllActivities()
        result = allActivities.map(activity => ({ ...activity, title: activity.name, moodTags: activity.tags }))
      }

      // Apply filters
      if (selectedFilter === 'trending') {
        result = result.filter(activity => (activity.rating && activity.rating >= 4.0) || activity.tags?.includes('popular'))
      } else if (selectedFilter === 'nearby') {
        result = result.filter(activity => activity.location || activity.tags?.includes('local'))
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white/80">
                Drag activities to your timeline or click + to add
              </p>
            </div>
            <button
              onClick={onClose}
...
export default EnhancedFloatingActivityBrowser
